...
namespace WhoWantsToBeAMillionaire
{
    public partial class Form1 : Form
    {
        ...
        private void SelectAmount()
        {
            ...
            // Получите выбранный элемент из SelectionForm
            FirelessAmountIndex = selectionForm.SelectedItem;


        }
        
        private void ReadQuestionsFromDatabase() // метод чтения данных из БД
        {
            string connectionString = "Host=localhost;Port=5432;Username=postgres;Password=admin;Database=WhoWantsToBeAMillionaire";

            using (NpgsqlConnection connection = new NpgsqlConnection(connectionString))
            {
                connection.Open();

                using (NpgsqlCommand cmd = new NpgsqlCommand("SELECT * FROM Questions", connection))
                {
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            // Создайте объект Question и добавьте его в коллекцию
        ...
        {
            // Проверьте, что индекс элемента действительный
            if (e.Index >= 0)
            {
                ListBox listBox = sender as ListBox;
                string itemText = listBox.Items[e.Index].ToString();

                // Создайте кисть для цвета текста
                Brush textBrush = Brushes.Black; // Черный цвет по умолчанию

                if (e.Index == FirelessAmountIndex) // Установите цвет текста для элемента с индексом
                {
                    textBrush = Brushes.Red; // Красный цвет для элемента
                }
